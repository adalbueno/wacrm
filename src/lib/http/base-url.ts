// ============================================================
// Resolve the base URL this app is reachable at, for building any
// absolute link the server hands back to a client (invite links,
// inbound-webhook-trigger URLs, ...). Extracted out of
// src/app/api/account/invitations/route.ts (issue: needed by a second
...
//   receive a generated link pointing at their site.
//
...
//
//   The env var name stays `ALLOWED_INVITE_HOSTS` even though this
//   module now backs more than just invite links — renaming it would
//   silently break any fork that already set it.
// ============================================================
...
export function getBaseUrl(request: Request): string {
  ...
  if (allowList && (forwardedHost || host)) {
    console.warn('[getBaseUrl] rejected non-allow-listed host:', {
      forwardedHost,
      host,
      allowList,
    });
